...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        ...
        pass
    try:
        from agent.claude_code_cli_client import AsyncClaudeCodeCLIClient, ClaudeCodeCLIClient
        if isinstance(sync_client, ClaudeCodeCLIClient):
            # Claude Code CLI is a synchronous subprocess transport.  Async
            # auxiliary callers (vision, title generation, compression, etc.)
            # still need an awaitable ``chat.completions.create`` surface, so
            # wrap the subprocess call in ``asyncio.to_thread`` instead of
            # returning the sync facade directly.
...
def resolve_provider_client(
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "claude-code-cli":
            ...
            base_url = str(creds.get("base_url", "")).strip() or "claude-code-cli://claude"
            command = str(creds.get("command", "")).strip() or None
            args = list(creds.get("args") or [])
            if not final_model:
                logger.warning(
