...
def get_model_context_length(
    ...
    # Claude Code CLI is a subscription-backed local process reached through a
    # marker URL (claude-code-cli://claude) that cannot be probed like an HTTP
    # endpoint.  Resolve the model's real advertised window from the hardcoded
    # catalog -- the same values every other surface uses -- instead of forcing
    # a special lower cap.  If a given account/session actually rejects long
    # prompts, cap it explicitly via config.yaml model.context_length or the
    # CLAUDE_CODE_CLI_CONTEXT_LENGTH env knob (both win at step 0 above).
    if provider == "claude-code-cli" or str(base_url or "").startswith("claude-code-cli://"):
        model_lower = _strip_provider_prefix(model or "").lower()
