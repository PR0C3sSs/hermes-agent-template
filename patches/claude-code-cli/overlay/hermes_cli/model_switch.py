    ...
    *,
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if overlay.auth_type == "aws_sdk":
            ...
            # External-process providers (Claude Code CLI, Copilot ACP) do not
            # advertise API-key env vars and may not have an auth-store entry.
            # Their availability is determined by the command/auth probe in
            # auth.get_auth_status(); without this branch they are callable via
            # provider:model but disappear from the interactive /model picker.
            try:
                from hermes_cli.auth import get_auth_status
                for _key in (hermes_slug, pid):
                    if not _key:
                        continue
                    _status = get_auth_status(_key) or {}
                    if _status.get("logged_in") or _status.get("configured"):
                        has_creds = True
                        break
    ...
    for _cp in _canon_provs:
        ...
        # External-process providers have no API-key env var; check the live
        # command/auth probe so canonical entries stay visible in /model.
        if not _cp_has_creds and _cp_config and getattr(_cp_config, "auth_type", "") == "external_process":
            try:
                from hermes_cli.auth import get_auth_status
                _cp_status = get_auth_status(_cp.slug) or {}
                if _cp_status.get("logged_in") or _cp_status.get("configured"):
                    _cp_has_creds = True
            except Exception:
                pass
