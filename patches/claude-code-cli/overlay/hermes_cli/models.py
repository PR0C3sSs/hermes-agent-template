...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    "claude-code-cli": [
        # Keep this in sync with Claude Code CLI's accepted --model slugs.
        # Claude Code has no /models endpoint, so provider_model_ids() must use
        # this curated local allowlist. Account-specific/date-suffixed variants
        # can still be typed directly with /model <slug>.
        "claude-fable-5",
        "claude-opus-4-8",
        "claude-opus-4-7",
        "claude-opus-4-6",
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("claude-code-cli", "Claude Code CLI",          "Claude Code CLI (spawns local `claude -p` and reuses Claude Max login)"),
...
_PROVIDER_ALIASES = {
    ...
    "claude-code-max": "claude-code-cli",
    "deep-seek": "deepseek",
